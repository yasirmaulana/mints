    ...
    <div style="padding-top:88px">
      ...
      <div class="px-5 py-16 md:px-12" style="border-bottom:1px solid rgba(9,11,12,0.08)">
        <div class="mx-auto max-w-5xl">
          <div class="mb-4 flex items-center gap-3">
            ...
            <span class="text-xs font-normal uppercase tracking-[0.16rem]" style="color:rgba(9,11,12,0.5)">Legal</span>
          </div>
          <h1 class="text-[2.5rem] font-normal leading-tight tracking-tight md:text-[4rem]">
            Syarat &<br><em style="font-style:italic;opacity:0.5">Ketentuan</em>
          </h1>
          <p class="mt-4 text-sm max-w-lg leading-relaxed" style="color:rgba(9,11,12,0.55)">
            Harap baca syarat dan ketentuan berikut dengan saksama sebelum menggunakan situs web dan layanan ini. Dengan mengakses atau membeli produk dari situs ini, Anda setuju untuk terikat oleh ketentuan di bawah ini.
          </p>
        </div>
      </div>
      ...
      <section class="px-5 py-16 md:px-12">
        <div class="mx-auto max-w-3xl space-y-4">
          <div v-for="(section, i) in sections" :key="i" class="rounded-3xl overflow-hidden" style="background:white">
            <div class="px-8 py-6 flex items-start gap-5" style="border-bottom:1px solid rgba(9,11,12,0.06)">
              <span class="flex-shrink-0 w-8 h-8 rounded-2xl flex items-center justify-center text-xs font-bold mt-0.5" style="background:#090b0c;color:white">{{ i + 1 }}</span>
              <h2 class="text-base font-semibold tracking-tight pt-1">{{ section.title }}</h2>
            </div>
            <ul v-if="section.items" class="divide-y" style="border-color:rgba(9,11,12,0.06)">
              <li v-for="(item, j) in section.items" :key="j" class="px-8 py-5 flex items-start gap-4">
                <span class="flex-shrink-0 w-1.5 h-1.5 rounded-full mt-2" style="background:#fabc3f" />
                <p class="text-sm leading-[1.9]" style="color:rgba(9,11,12,0.65)" v-html="item" />
              </li>
            </ul>
            <div v-else class="px-8 py-5">
              <p class="text-sm leading-[1.9]" style="color:rgba(9,11,12,0.65)" v-html="section.body" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="px-5 pb-24 md:px-12">
        <div class="mx-auto max-w-5xl">
          <div class="rounded-3xl px-10 py-14 text-center" style="background:#090b0c">
            <p class="text-xs uppercase tracking-widest mb-3" style="color:rgba(255,255,255,0.4)">Ada pertanyaan?</p>
      ...
      <div class="mx-auto max-w-5xl">
        ...
        <div class="border-t pt-8 flex flex-col sm:flex-row items-center justify-between gap-4" style="border-color:rgba(9,11,12,0.1)">
          ...
          <div class="flex gap-4 text-xs" style="color:rgba(9,11,12,0.4)">
            ...
            <NuxtLink to="/syarat-ketentuan" class="hover:text-[#090b0c] transition-colors font-medium" style="color:#090b0c">Syarat & Ketentuan</NuxtLink>
          </div>
        </div>
      </div>
    </footer>
