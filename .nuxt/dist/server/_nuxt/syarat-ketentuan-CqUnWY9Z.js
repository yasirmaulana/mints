...
const _sfc_main = /* @__PURE__ */ defineComponent({
  __name: "syarat-ketentuan",
  __ssrInlineRender: true,
  setup(__props) {
    useHead({ title: "Syarat & Ketentuan — Mints" });
    const { isLoggedIn } = useAuth();
    const { itemCount } = useCart();
    const mobileMenuOpen = ref(false);
    const sections = [
      {
        title: "Pemesanan & Harga",
        items: [
          "Semua produk bergantung pada ketersediaan. Kami berhak membatalkan pesanan jika produk tidak tersedia.",
          "Harga yang tercantum pada situs ini adalah dalam <strong>Rupiah (IDR)</strong> dan dapat berubah sewaktu-waktu tanpa pemberitahuan sebelumnya, namun tidak akan memengaruhi pesanan yang sudah dibayar dan terkonfirmasi."
        ]
      },
      {
        title: "Pembayaran",
        items: [
          "Kami menerima pembayaran melalui metode yang tersedia di halaman checkout melalui payment gateway rekanan Mints.",
          "Jika pembayaran tidak diterima atau tidak dikonfirmasi dalam batas waktu pembayaran, sistem berhak <strong>membatalkan pesanan secara otomatis</strong>."
        ]
      },
      {
        title: "Hak Kekayaan Intelektual",
        body: "Seluruh konten pada situs web ini — termasuk namun tidak terbatas pada tulisan, logo, gambar, desain, ikon, dan perangkat lunak — adalah <strong>properti milik Mints</strong> atau pemasok kontennya dan dilindungi oleh undang-undang yang berlaku."
      },
      {
        title: "Informasi Produk (Warna & Ukuran)",
        body: "Mints telah berupaya sebaik mungkin untuk menampilkan warna produk seakurat mungkin. Namun, karena perbedaan pada setiap layar, kami tidak dapat menjamin bahwa tampilan warna pada perangkat Anda akan <strong>100% sama</strong> dengan produk aslinya."
      }
    ];
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</header><div style="${ssrRenderStyle({ "padding-top": "88px" })}"><div class="px-5 py-16 md:px-12" style="${ssrRenderStyle({ "border-bottom": "1px solid rgba(9,11,12,0.08)" })}"><div class="mx-auto max-w-5xl"><div class="mb-4 flex items-center gap-3"><div class="h-px w-5" style="${ssrRenderStyle({ "background": "rgba(9,11,12,0.6)" })}"></div><span class="text-xs font-normal uppercase tracking-[0.16rem]" style="${ssrRenderStyle({ "color": "rgba(9,11,12,0.5)" })}">Legal</span></div><h1 class="text-[2.5rem] font-normal leading-tight tracking-tight md:text-[4rem]"> Syarat &amp;<br><em style="${ssrRenderStyle({ "font-style": "italic", "opacity": "0.5" })}">Ketentuan</em></h1><p class="mt-4 text-sm max-w-lg leading-relaxed" style="${ssrRenderStyle({ "color": "rgba(9,11,12,0.55)" })}"> Harap baca syarat dan ketentuan berikut dengan saksama sebelum menggunakan situs web dan layanan ini. Dengan mengakses atau membeli produk dari situs ini, Anda setuju untuk terikat oleh ketentuan di bawah ini. </p></div></div><section class="px-5 py-16 md:px-12"><div class="mx-auto max-w-3xl space-y-4"><!--[-->`);
      ssrRenderList(sections, (section, i) => {
        _push(`<div class="rounded-3xl overflow-hidden" style="${ssrRenderStyle({ "background": "white" })}"><div class="px-8 py-6 flex items-start gap-5" style="${ssrRenderStyle({ "border-bottom": "1px solid rgba(9,11,12,0.06)" })}"><span class="flex-shrink-0 w-8 h-8 rounded-2xl flex items-center justify-center text-xs font-bold mt-0.5" style="${ssrRenderStyle({ "background": "#090b0c", "color": "white" })}">${ssrInterpolate(i + 1)}</span><h2 class="text-base font-semibold tracking-tight pt-1">${ssrInterpolate(section.title)}</h2></div>`);
        if (section.items) {
          _push(`<ul class="divide-y" style="${ssrRenderStyle({ "border-color": "rgba(9,11,12,0.06)" })}"><!--[-->`);
          ssrRenderList(section.items, (item, j) => {
            _push(`<li class="px-8 py-5 flex items-start gap-4"><span class="flex-shrink-0 w-1.5 h-1.5 rounded-full mt-2" style="${ssrRenderStyle({ "background": "#fabc3f" })}"></span><p class="text-sm leading-[1.9]" style="${ssrRenderStyle({ "color": "rgba(9,11,12,0.65)" })}">${item ?? ""}</p></li>`);
          });
          _push(`<!--]--></ul>`);
        } else {
          _push(`<div class="px-8 py-5"><p class="text-sm leading-[1.9]" style="${ssrRenderStyle({ "color": "rgba(9,11,12,0.65)" })}">${section.body ?? ""}</p></div>`);
        }
        _push(`</div>`);
      });
      _push(`<!--]--></div></section><section class="px-5 pb-24 md:px-12"><div class="mx-auto max-w-5xl"><div class="rounded-3xl px-10 py-14 text-center" style="${ssrRenderStyle({ "background": "#090b0c" })}"><p class="text-xs uppercase tracking-widest mb-3" style="${ssrRenderStyle({ "color": "rgba(255,255,255,0.4)" })}">Ada pertanyaan?</p><h2 class="text-2xl font-normal tracking-tight text-white mb-6 md:text-3xl">Hubungi kami kapan saja.</h2><a href="mailto:hello@mints.id" class="inline-flex items-center gap-2 rounded-full px-7 py-3.5 text-sm font-normal transition-opacity hover:opacity-85" style="${ssrRenderStyle({ "background": "white", "color": "#090b0c" })}"> hello@mints.id <span class="flex h-5 w-5 items-center justify-center rounded-full text-xs" style="${ssrRenderStyle({ "background": "#090b0c", "color": "white" })}">→</span></a></div></div></section></div><footer class="overflow-hidden px-5 pt-20 pb-10 md:px-12" style="${ssrRenderStyle({ "background": "#f5f5f2", "border-top": "1px solid rgba(9,11,12,0.08)" })}"><div class="mx-auto max-w-5xl"><div class="grid gap-12 sm:grid-cols-3 mb-12"><div><p class="font-black text-xl tracking-tighter mb-3" style="${ssrRenderStyle({ "font-family": "'Inter Tight',sans-serif", "letter-spacing": "-0.04em" })}">MINTS</p><p class="text-sm leading-6" style="${ssrRenderStyle({ "color": "rgba(9,11,12,0.5)" })}">Elegan. Syar&#39;i.<br>Dibuat Sepenuh Hati.</p></div><div><p class="mb-4 text-xs font-normal uppercase tracking-[0.16rem]" style="${ssrRenderStyle({ "color": "rgba(9,11,12,0.4)" })}">Belanja</p><ul class="space-y-3 text-sm" style="${ssrRenderStyle({ "color": "rgba(9,11,12,0.6)" })}"><li>`);
      _push(ssrRenderComponent(_component_NuxtLink, {
        to: "/koleksi",
      ...
      _push(ssrRenderComponent(_component_NuxtLink, {
        to: "/syarat-ketentuan",
        class: "hover:text-[#090b0c] transition-colors font-medium",
        style: { "color": "#090b0c" }
      }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/syarat-ketentuan.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
export {
  _sfc_main as default
};
//# sourceMappingURL=syarat-ketentuan-CqUnWY9Z.js.map
